...
void ofApp::setup(){
    ...
    secondWindow.setup("chessboard", -PROJECTOR_RESOLUTION_X, 0, fboChessboard.getWidth(), fboChessboard.getHeight(), true);
    secondWindow.show();
...
void ofApp::drawTestingPoint(ofVec2f projectedPoint) {
    ...
    ofBackground(255);
    ofSetColor(0, 255, 0);
    ofDrawCircle(ofMap(projectedPoint.x, 0, 1, 0, fboChessboard.getWidth()),
...
void ofApp::keyPressed(int key){
    if (key=='z') {
        searching = !searching;
        ofLogNotice() << (searching ? "searching" : "not searching");
    }
    if (key==' '){
        addPointPair();
    } else if (key=='q') {
        chessboardSize -= 20;
        ofLogNotice() << chessboardSize;
    } else if (key=='w') {
        chessboardSize += 20;
        ofLogNotice() << chessboardSize;
    } else if (key=='c') {
